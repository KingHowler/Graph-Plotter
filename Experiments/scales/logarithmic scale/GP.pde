          ...
          if (SubGrid) {
            //print("Begin \n");
              ...
              for (float i = XMin; i < XMax; i+= XStep/10) {
                  ...
                  float x = map(sc, exp(XMin), exp(XMax), 100, WidthP - 50);
    ...
    void DrawGraph(float[] graph, color graphColor) {
      ...
      for (int i = 0; i < totalPoints - 1; i++) {
        ...
        // Get coordinate of current value
          x1 = map(exp(x(i)), exp(x(0)), exp(x(totalPoints - 1)), 100, WidthP - 50);
          print(x(i) + "   " + exp(x(i)) + "   " + exp(x(0)) + "    "  + exp(x(totalPoints - 1)) + "\n");
          y1 = yCo(graph[i]);
  
          // Get coordinate of next value
          x2 = map(exp((x(i + 1))), exp(x(0)), exp(x(totalPoints - 1)), 100, WidthP - 50);
          y2 = yCo(graph[i + 1]);
