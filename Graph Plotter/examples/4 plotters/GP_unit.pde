public class GraphPlotter {
    ...
    void DrawGrid() {
        ...
        stroke(255);
        fill(bg);
        rect(OffsetX, (HeightP - 50) + OffsetY, (WidthP) + OffsetX, (50) + OffsetY);
        ...
        if (SubGrid) {
            ...
            for (float i = XMin; i < XMax; i+= XStep/10) {
                ...
                int x = int(map(i, XMin, XMax, 100, WidthP));

                // Draw Vertical Line
                strokeWeight(0.5);
                stroke(200);  // Light gray color for subgrid
                line((x) + OffsetX, (HeightP - 50) + OffsetY, (x) + OffsetX, OffsetY);
            }
            
            // X sub grid
            for (float i = YMin; i < YMax + 1; i+= YStep/10) {
                // Map each y value to a coordinate
                int y = int(map(i, YMin, YMax, HeightP - 50, 0));

                // Draw Horizontal Line
                strokeWeight(0.5);
                stroke(200);  // Light gray color for subgrid
                line((100) + OffsetX, (y) + OffsetY, (WidthP) + OffsetX, (y) + OffsetY);
            }
        }
        // Draw Main Grid
        // Y grid
        ...
        // X axis
        int y = int(map(0, YMin, YMax, HeightP - 50, 0));
        textAlign(RIGHT);
        text(String.valueOf(0), (90) + OffsetX, (y) + OffsetY);
        line((100) + OffsetX, (y) + OffsetY, (WidthP) + OffsetX, (y) + OffsetY);
        
        // Y axis
        int x = int(map(0, XMin, XMax, 100, WidthP));
        textAlign(CENTER);
    ...
    stroke(graphColor);
      for (int i = 0; i < totalPoints - 1; i++) {
        ...
        float x1 = map(i, 0, totalPoints - 1, 100, WidthP);
        float y1 = map(graph[i], YMin, YMax, HeightP - 50, 0);

        // Get coordinate of next value
        float x2 = map(i + 1, 0, totalPoints - 1, 100, WidthP);
        float y2 = map(graph[i + 1], YMin, YMax, HeightP - 50, 0);
